...
export class OutlookController {
  ...
  syncEmails = async (_req: Request, res: Response) => {
    ...
    try {
      const integration: any = await integrationsService.getOutlookIntegration();
      if (!integration.success) {
        throw new BadRequestError("Unable to get outlook integration");
      }
      ...
      for (const integration of integrations) {
        ...
        try {
          ...
          const attachments = await outlookServices.getEmailsWithAttachments(
            tokens,
            integration.userId,
            integration.id,
            lastRead,
  ...
  syncMyEmails = async (req: Request, res: Response) => {
    try {
      ...
      if (!integration || integration.status !== "success") {
        throw new BadRequestError("Outlook integration not found or not connected");
      }
  ...
  getAttachments = async (req: Request, res: Response) => {
    ...
    try {
      const attachmentsData = await outlookServices.getAttachments(
        userId,
        page,
        limit
      );
  ...
  getAttachmentWithId = async (req: Request, res: Response) => {
    try {
      ...
      const response = await outlookServices.getAttachmentWithId(id, userId);
