...
async function handleRequest(request: NextRequest, method: string) {
  try {
    ...
    if (backendResponse.status === 200) {
      ...
      redirectUrl.searchParams.set("type", "integration.outlook");
      redirectUrl.searchParams.set("message", "Outlook successfully integrated");
      return NextResponse.redirect(redirectUrl);
    }

    // Handle error responses
    const errorData = await backendResponse.json().catch(() => ({}));
    const errorMessage = errorData.message || "Failed to connect Outlook";
    const redirectUrl = new URL("/integrations", request.url);
